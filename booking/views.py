...
def buy_my_item(request):
    ...
    ppp = PayPalPro(
              item=item,                            
              payment_template="booking/credit.html",      
...
def thank(request):
    ...
    # b=Booking.objects.get(customer_id = request.session['customer_id'])
    a=PayPalNVP.objects.last()
    print(a.response)
    # print(b)
    d=a.response
    trans_id=d.split("M&transactionid=")[1]
    print(trans_id)
    booking_id = request.session['booking_id']
    print("booking_id",booking_id)
    return render(request,'booking/thank_you.html',{"data":trans_id,"book":booking_id})
